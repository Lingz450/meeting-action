...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const viewerData = await viewer.json();
    const organization = viewerData.data?.viewer?.organization;
    ...
    if (dbError) {
      console.error('Failed to store Linear integration:', dbError);
      throw new Error('Failed to store integration');
    }

    // Success
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/integrations?success=linear`
    );
  } catch (error) {
